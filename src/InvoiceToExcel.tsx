...
async function extractInvoiceData(entry: InvoiceEntry) {
  const pageNote = entry.pagesNote
    ? `\n\nNote : seules les ${entry.pagesNote} premières pages du document t'ont été fournies (document trop volumineux pour être envoyé en entier). S'il manque des articles au-delà, ce n'est pas grave, extrais ce qui est visible.`
    : "";
...
dans une seule liste, sans doublons, sans recopier les lignes de total de page intermédiaire.${pageNote}
...
N'invente aucune donnée. Si un champ n'est pas lisible, mets une chaîne vide ou 0.`;
  ...
  const response = await fetch("/api/extract", {
    ...
    body: JSON.stringify({ base64: entry.base64, mimeType: entry.mimeType, images: entry.images, prompt }),
  });
...
export default function InvoiceToExcel() {
  ...
  const processAll = async () => {
    ...
    for (const entry of pending) {
      ...
      try {
        const result = await extractInvoiceData(entry);
        setEntries((prev) =>

...
import { prepareFileForApi } from "./pdfUtils";
import {
  Upload,
  Loader2,
  Trash2,
...
async function callExtract(
  prompt: string,
  payload?: { base64?: string; mimeType?: string; images?: { base64: string; mimeType: string }[] }
) {
...
async function extractSpecs(sheet: Sheet) {
  const pageNote = sheet.pagesNote
    ? `\n\nNote : seules les ${sheet.pagesNote} premières pages du document t'ont été fournies (document trop volumineux pour être envoyé en entier). Les caractéristiques techniques figurent généralement dans les premières pages ; si tu ne trouves pas certaines informations, laisse-les vides plutôt que de les inventer.`
    : "";
...
- TOUTES les caractéristiques techniques présentes sur la fiche, sous forme de paires clé/valeur, avec l'unité incluse
  ...
  inclus tout ce qui figure réellement sur la fiche, même si ce n'est pas dans cette liste d'exemples.${pageNote}
...
N'invente aucune donnée. Si un champ n'est pas lisible, mets une chaîne vide.`;
  ...
  const text = await callExtract(prompt, { base64: sheet.base64, mimeType: sheet.mimeType, images: sheet.images });
...
export default function DatasheetCompare() {
  ...
  const addFiles = useCallback(async (files: FileList | null) => {
    ...
    for (const file of Array.from(files)) {
      ...
      const prepared = await prepareFileForApi(file);
      newSheets.push({
        id: uid(),
        fileName: file.name,
        ...
        pagesNote:
          prepared.pagesUsed && prepared.pagesTotal && prepared.pagesUsed < prepared.pagesTotal
            ? `${prepared.pagesUsed}/${prepared.pagesTotal}`
            : undefined,
        status: "pending",
        productName: "",
        reference: "",
  ...
  const processAll = async () => {
    ...
    for (const sheet of pending) {
      ...
      try {
        const result = await extractSpecs(sheet);
          ...
          >
            ...
            <div className="flex-1 min-w-0">
              ...
              <p className="text-xs text-gray-500 truncate">
                ...
                {sheet.status === "done" &&
                  `${sheet.productName || "—"}${sheet.reference ? " · " + sheet.reference : ""} · ${sheet.specs.length} caractéristique(s)${sheet.pagesNote ? ` · ${sheet.pagesNote} pages analysées` : ""}`}
